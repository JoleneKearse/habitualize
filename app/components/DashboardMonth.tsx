...
type HabitLogWithHabit = DashboardMonthProps["habitLogMonth"][0];
...
export default function DashboardMonth({ habitLogMonth }: DashboardMonthProps) {
  const now = new Date();
  ...
  const logsByDate = groupLogsByISODate(habitLogMonth);
  ...
  return (
    <section className="flex flex-col items-center justify-center gap-4 px-2 text-gray-800 dark:text-gray-200 h-screen w-full pb-10">
      <h2 className="text-3xl font-bold bg-linear-[90deg,#e11d48,#c026d3,#7c3aed,#0284c7,#16a34a,#eab308,#ea580c,#dc2626] text-transparent bg-clip-text pt-24">
        {month} {year}
      </h2>
